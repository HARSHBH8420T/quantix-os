...
if 'recovered_count' not in st.session_state:
    st.session_state.recovered_count = 0
if 'failed_count' not in st.session_state:
    st.session_state.failed_count = 0
if 'last_net_settlement' not in st.session_state:
    st.session_state.last_net_settlement = 0

raw = run_reconciliation(batch_size=60)
matched_list = raw['matched']
exception_list = raw['exceptions']
df_all = pd.DataFrame(matched_list + exception_list)
...
if menu == "📊 Dashboard":
    ...
    if st.session_state.llm_calls >= 20:
        st.error("Rate limit: 20 queries per session. Refresh page to reset.")
        st.stop()
    if not final_groq_key:
        st.error("GROQ_API_KEY missing. Add it to.streamlit/secrets.toml")
        st.stop()
    ...
    if q:
        ...
        filtered_df = df_f
        if found:
            filtered_df = filtered_df[filtered_df['bank'].str.lower()==found]
        if any(k in q_low for k in ["atka","delay","mismatch","exception","kyun","kyu","pending"]):
            filtered_df = filtered_df[filtered_df['status']=='Exception']
        ...
        for r in context_rows:
            ...
            mdr = r.get('mdr', 0)
            gst = r.get('gst', 0)
            net = gross - mdr - gst if gross else 0
            clean_context.append({
                "settlement_id": r.get('settlement_id', f"SETL_{r.get('bank')}_{random.randint(100,999)}"),
                "utr": r.get('utr'),
                "bank": r.get('bank'),
        ...
        system_prompt = f"You are Quantix CFO Auditor for RazorpayX. You MUST use provided transaction context. Rules: 1. Reply in SAME language as user asked (Hinglish if user uses Hinglish) 2. Always mention settlement_id, UTR, merchant, amounts, MDR, GST, net_settlement 3. Give root cause from reason field 4. End with 1-line Action. Context: {clean_context}"
        try:
            ...
            st.success(f"🤖 LLM: {ans}")
            if clean_context:
                c1, c2 = st.columns(2)
                c1.info(f"📎 Source: {clean_context[0].get('settlement_id')}")
                c2.info(f"🎯 Confidence: {random.randint(88,97)}%")
...
elif menu == "🏦 Bank Feeds":
    st.header("🏦 Live Bank Feeds")
    st.dataframe(df_f[['date','bank','utr','gross_amount','status','reason']], width="stretch")

elif menu == "🧾 Tax Ledger":
    st.header("🧾 MDR + GST Ledger")
    st.dataframe(df_f[['date','bank','merchant','utr','gross_amount','mdr','gst','status']], width="stretch")
    c1,c2,c3 = st.columns(3)
    c1.metric("Total MDR", f"Rs {df_f['mdr'].sum():,.2f}")
    c2.metric("Total GST", f"Rs {df_f['gst'].sum():,.2f}")
    c3.metric("Net", f"Rs {(df_f['mdr'].sum()+df_f['gst'].sum()):,.2f}")
    ...
    st.header("📈 AI Revenue Forecast")
    total_rev = df_f['gross_amount'].sum()
    future, growth = ai_forecast_revenue(total_rev)
    c1,c2 = st.columns(2)
    ...
    with c1:
        ...
        if st.button("💥 Simulate Webhook FAIL", width="stretch", type="primary"):
            ...
            with st.spinner("🤖 AI Agent retrying with exponential backoff..."):
                ...
                if result['status']==200 and not result.get('duplicate'):
                    st.session_state.recovered_count += 1
                    st.session_state.last_net_settlement = result['ai']['net_amount']
                elif result['status']==500:
                    st.session_state.failed_count += 1
            st.rerun()
        if st.button("🔁 Test Duplicate (Idempotency)", width="stretch"):
            if 'last_payload' in st.session_state:
                result = process_webhook_secure(st.session_state['last_payload'])
                st.session_state['last_result'] = result
                if result.get('duplicate'):
                    st.toast("Duplicate blocked - Idempotency working!")
                st.rerun()
            else:
                st.warning("Pehle ek FAIL simulate karo")
    ...
    with c2:
        ...
        if 'last_result' in st.session_state and st.session_state['last_result']:
            ...
            if res['status']==200 and not res.get('duplicate'):
                gross = st.session_state['last_payload']['amount']
        ...
        dlq = get_dlq()
        if dlq:
            st.dataframe(pd.DataFrame(dlq), width="stretch")
        else:
            st.caption("No failures yet - DLQ empty (Good!)")
